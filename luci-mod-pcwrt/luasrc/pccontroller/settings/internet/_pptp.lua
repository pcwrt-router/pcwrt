...
module("luci.pccontroller.settings.internet._pptp", package.seeall)

function validate(v)
    local errs = {}

    if string.is_empty(v.server) then
	errs.server = i18n.translate('Please enter VPN server address')
    else
	if not dt.host(v.server) then
	    errs.server = i18n.translate('Invalid VPN server address')
	end
    end
