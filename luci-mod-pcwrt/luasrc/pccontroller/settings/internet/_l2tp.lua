...
module("luci.pccontroller.settings.internet._l2tp", package.seeall)

function validate(v)
    local errs = {}

    if string.is_empty(v.server) then
	errs.server = i18n.translate('Please enter the L2TP server address')
    else
	if not dt.ip4addr(v.server) then
	    errs.server = i18n.translate('Invalid L2TP server address')
	end
    end
